...
nut.command.add("charsetname", {
	...
	onRun = function(client, arguments)
		local target = nut.command.findPlayer(client, arguments[1])
		if (!client:getChar():hasFlags("F")) then return end
...
nut.command.add("dropmoney", {
	...
	onRun = function(client, arguments)
		...
		amount = math.Round(amount)
		local items = client:getChar():getInv():hasItem("kowelek")
		if (!items) then
		...
		client:ConCommand("say /zlostmoney " ..amount.. " руб")
		client:getChar():takeMoney(amount)
...
--[[nut.command.add("dropmoney", {
	...
	onRun = function(client, arguments)
		...
		client:ConCommand("say /322lostmoney " ..amount.. " руб")
		client:getChar():takeMoney(amount)
		local money = nut.currency.spawn(client:getItemDropPos(), amount)
		money.client = client
		money.charID = client:getChar():getID()
...
nut.command.add("plytransfer", {
	...
	syntax = "<string name> <string faction>",
	onRun = function(client, arguments)
		if (!client:getChar():hasFlags("F")) then return end --  нету флагв
		local target = nut.command.findPlayer(client, arguments[1])
		local name = table.concat(arguments, " ", 2)
...
nut.command.add("rep", {
	...
	onRun = function(client, arguments)
		...
		---Медаль
		--[[if (client:getChar():getData("rep", 0) >= 100) then
			client:getChar():setData("rep", attacker:getChar():getData("rep", 0) - 100)
		end]]
	end
})
...
nut.command.add("rank", {
	...
	onRun = function(client, arguments)
		...
		---Медаль
		--[[if (client:getChar():getData("rank", 0) >= 10000) then
			--client:getChar():setData("rank", client:getChar():getData("rank", 0) - 10000)
			client:getChar():getInv():add("m1", 1)
		end
		if (client:getChar():getData("rank", 0) >= 2700) then
			--client:getChar():setData("rank", client:getChar():getData("rank", 0) - 2700)
			client:getChar():getInv():add("m1", 1)
		end
		if (client:getChar():getData("rank", 0) >= 700) then
			--client:getChar():setData("rank", client:getChar():getData("rank", 0) - 700)
			client:getChar():getInv():add("m1", 1)
		end
		if (client:getChar():getData("rank", 0) >= 110) then
			--client:getChar():setData("rank", client:getChar():getData("rank", 0) - 110)
			client:getChar():getInv():add("m1", 1)
		end
		if (client:getChar():getData("rank", 0) >= 100) then
			--client:getChar():setData("rank", client:getChar():getData("rank", 0) - 100)
			client:getChar():getInv():add("m1", 1)
		end]]
		--[[if (client:getChar():getData("rank", 0) >= 99) then
			client:getChar():setData("rank", attacker:getChar():getData("rank", 0) - 100)
			client:getChar():getInv():add("m1", 1)
		end]]
	end
})
...
--[[nut.command.add("charsetrank", {
	adminOnly = true,
	syntax = "<string character> <number rank>",
	onRun = function(client, arguments)
...
--[[nut.command.add("charsetrep", {
	adminOnly = true,
	syntax = "<string character>",
	onRun = function(client, arguments)
		local target = nut.util.findPlayer(arguments[1])
		--local rep = tonumber(arguments[2])

		target:setData("rep", target:getChar():getData("rep", 0) + 50)
