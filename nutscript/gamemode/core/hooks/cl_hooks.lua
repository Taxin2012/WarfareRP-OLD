...
--[[hook.Add("GetPlayerIcon", function()
	local playericon = "stalkerrp/face.png"  
		...
		elseif (LocalPlayer():GetModel() == "models/spenser/ssk/new_free_03_spen.mdl") then
            playericon = "stalkerrp/face16.png"
		...
		elseif (LocalPlayer():GetModel() == "models/wicked/squad/wick_brodyga.mdl") then
 		...
 		elseif (LocalPlayer():GetModel() == "models/wicked/squad/wick_sunrice_01.mdl") then
            playericon = "stalkerrp/wickico.png"		    
        else
        	playericon = "stalkerrp/face310.png"    		
        end

    return playericon
end)]]

hook.Add("GetPlayerIcon1", "nutPlayerIcon1", function()
	local material1 = "stalkerrp/face.png"  
...
--[[function GM:GetRepText(client)
	local rep = client:getChar():getData("rep", 0)
	--local reps = "Репутация: Безразлично"
	--if (rep > 100 and rep < 199) then
	
	if (rep > 2000 and rep < 999999999) then
		reps = "Репутация: Великолепно"
		
	elseif (rep > 1000 and rep < 1999) then
		reps = "Репутация: Отлично"
		
	elseif (rep > 500 and rep < 999) then
		reps = "Репутация: Оч. Хорошо"
		
	elseif (rep > 100 and rep < 499) then
		reps = "Репутация: Хорошо"
		
	elseif (rep > -29 and rep < 99) then
		reps = "Репутация: Безразлично"
		
	elseif (rep > -79 and rep < -30) then
		reps = "Репутация: Плохо"
		
	elseif (rep > -299 and rep < -80) then
		reps = "Репутация: Оч. плохо"
		
	elseif (rep > -599 and rep < -300) then
		reps = "Репутация: Ужасно"
		
	elseif (rep > -9999999 and rep < -600) then
		reps = "Репутация: Хуже всех"
		
	end
	return rep
end]]

--[[function GM:GetRankText(client)
	local rank = client:getChar():getData("rank", 0)
	--local ranks = "Новичок"
	--if (rank > 100 and rank < 199) then
	
	if (rank > 12000 and rank < 999999999) then
		ranks = "Легенда"
	...
	elseif (rank > 2000 and rank < 3199) then
		ranks = "Ветеран"
	...
	elseif (rank > 0 and rank < 199) then
		ranks = "Новичок"
	end
	return ranks
end]]
...
function GM:DrawCharInfo(client, character, info)---эта функция там есть, просто ее замени на эту 
local injText, injColor = hookRun("GetInjuredText", client) 
--[[local rep = client:getChar():getData("rep", 0) 
local reps = client:getNetVar("REP", "Репутация") 
--if (rep > 100 and rep < 199) then 

if (rep > 2000) then 
reps = "Репутация: Безразлично" 
client:setNetVar("REP",reps) 
elseif (rep > 1000 and rep < 1999) then 
reps = "Репутация: Отлично" 
client:setNetVar("REP",reps) 
elseif (rep > 500 and rep < 999) then 
...
local ranktext = hookRun("GetRankText", client) 
if (client:getNetVar("REP", "Репутация") != nil) then 
info[#info + 1] = {client:getNetVar("REP", "Репутация"), Color(230,255,25)} 
end 
if (ranktext) then 
