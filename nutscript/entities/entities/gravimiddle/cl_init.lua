...
function ENT:Think()--1
	for k, v in pairs( ents.FindInSphere( self:GetPos(), 120 ) ) do	
			if (v:GetPos():Distance(self:GetPos()) <= 85) then
				if (string.find( v:GetClass(), "prop" ) or v:IsRagdoll() or v:GetModel() == "models/weapons/stalker_cop/w_bolt.mdl") then
					if (!self.hit or self.hit < CurTime()) then
							self:EmitSound("anomaly/gravi_hit.wav")		
							local particle = self.Emitter:Add( self:getNetVar("eff3"), v:GetPos() )	
							...
							local smoke = self.Emitter:Add( self:getNetVar("eff2"), v:GetPos() )
		...
		if (v:GetPos():Distance(self:GetPos()) <= 85) then
			if (v:IsPlayer()) then
					if (!self.hit or self.hit < CurTime()) then
						...
						local particle = self.Emitter:Add( self:getNetVar("eff3"), v:GetPos() + Vector(0, 0, 40) )	
						particle:SetVelocity( Vector( 0, 0, 5 ) )  
		...
		if (self:getNetVar("start") == true) then
			local particle = self.Emitter:Add( self:getNetVar("eff3"), self:GetPos() + Vector( 0, 0, 5 ) )
			particle:SetVelocity( Vector( 0, 0, 5 ) )  
			...
			//
				if (!self.hit2 or self.hit2 < CurTime()) then
					local particle = self.Emitter:Add( self:getNetVar("eff1")..math.random(1, 8), self:getNetVar("pos") )
					particle:SetVelocity( Vector( 0, 0, math.random( 25, 35 ) ) )
		...
		if (self:getNetVar("start3") == true) then
			...
			particle:SetStartSize( math.random( 80, 95 ) ) 
			particle:SetEndSize( 0 ) 
			particle:SetColor( 255, 255, 255 )
		end
		//
