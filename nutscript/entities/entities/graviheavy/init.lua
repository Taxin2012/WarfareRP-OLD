...
function anomgraviheavy(ent)
  local self = ent
  self:setNetVar("eff5", "effects/leaves")
  self:setNetVar("eff4", "effects/pfx_anomaly3")
  self:setNetVar("eff3", "effects/spark")
  self:setNetVar("eff2", "effects/pfx_anomaly1")
   self:setNetVar("eff1", "effects/pfx_anomaly2")
    self:setNetVar("sound1", "anomaly/gravi_hit.wav")
      local vec = VectorRand()
      vec.z = -0.1  
   local newpos = self:GetPos()+Vector(math.random( -165, 165),math.random( -165, 165), 25) + vec * 55
    self:setNetVar("pos", newpos)
      local vec = VectorRand()
      vec.z = -0.1  
      local newpos2 = self:GetPos()+Vector(0,0,-700) + vec * 155
    ...
    if (self:getNetVar("start") == false) then
      ...
      if (!self.idleEF or self.idleEF < CurTime()) then
        ParticleEffect("gravyanomssk_2", self:GetPos()+Vector(math.random( -5, 5 ),math.random( -5, 5 ),-25), Angle(0,0,0), nil) 
        self.idleEF = CurTime() + 1.9
      end
      if (!self.idleEF2 or self.idleEF2 < CurTime()) then
        ParticleEffect("leavesssk_r_3", self:GetPos()+Vector(0,0,5), Angle(0,0,0), nil) 
        self.idleEF2 = CurTime() + 1.5
      end
    end
    ///////////////////////////////////////
      ...
      if (v:IsValid() and v:GetPhysicsObject():IsValid() and (!self.hit or self.hit < CurTime())) then
        if (string.find( v:GetClass(), "prop" )) then
          if (v:GetPos():Distance(self:GetPos()) <= 120) then
            if (v:getNetVar("usehit") == false) then
              v:setNetVar("usehit", true)   
              if (!v:IsRagdoll()) then
        ...
        if (v:GetPos():Distance(self:GetPos()) <= 115) then
           if (v:GetClass() == "npc_mutant_bloodsucker" or v:GetClass() == "ssk_boar_s" or v:GetClass() == "ssk_boar_m" or v:GetClass() == "ssk_tush" or v:GetClass() == "ssk_flash_s" or v:GetClass() == "ssk_flash_m" or v:GetClass() == "ssk_krovo_m" or v:GetClass() == "ssk_krovo_s" or v:GetClass() == "ssk_krovo_e" or v:GetClass() == "ssk_snork" or v:GetClass() == "ssk_izlom" or v:GetClass() == "ssk_zombie" or v:GetClass() == "ssk_himera") then
            local all = v
              ...
              rag:Spawn()
              rag:Activate()
              if (all:GetModel() == "models/wick/stalker/zombied/wick_bandit_models_v1_test.mdl") then
    ...
    if (!self.surge or self.surge < CurTime()) then
      ...
      ParticleEffect("dust", self:GetPos()+Vector(0,0,250), Angle(0,0,0), nil) 
      ...
      timer.Simple(2.3, function()
        ...
        if (v:IsValid() and v:GetPos():Distance(self:GetPos()) <= 120 and v:Health() > 0) then
          if (v:IsPlayer()) then
            v:TakeDamage(math.random(70, 80)) -- Player Damage
          elseif (v:IsNPC()) then
      ...
      Light:Fire("TurnOff", "", 1.4)  
          ...
          self:setNetVar("start3", false)
      local effectdata = EffectData()
            effectdata:SetStart( self:GetPos() + Vector( 0, 0, 250 )) 
            effectdata:SetOrigin( self:GetPos() + Vector( 0, 0, 250 ))
    ...
    if (v:GetPos():Distance(self:GetPos()) <= 300) then
      if (string.find( v:GetClass(), "prop_phys" ) or v:IsRagdoll()) then
        if (v:IsValid() and v:GetPhysicsObject():IsValid() ) then
         if (self:getNetVar("start") == true) then
          if (string.find( v:GetClass(), "prop_phys" )) then
            local pos = self:GetPos()+Vector(0, 0, 250)
            local vel = ( pos - v:GetPos() ):GetNormal()
            local phys = v:GetPhysicsObject()
            phys:ApplyForceCenter( vel * ( phys:GetMass() * 200 ) )
          elseif (v:IsRagdoll()) then
            local pos = self:GetPos()+Vector(0, 0, 250)
            local vel = ( pos - v:GetPos() ):GetNormal()
            local phys = v:GetPhysicsObject()
            phys:ApplyForceCenter( vel * ( phys:GetMass() * 1100 ) )
            end
          end
        end
      /////////////////
      ////////////////
    if (!self.surge or self.surge < CurTime()) then
      self:EmitSound("anomaly/anomaly_mincer_blowout.wav")
      self:setNetVar("start", true)
      timer.Simple(5.5, function()
        self:setNetVar("start3", true)
      end)
      timer.Simple(6, function() -- Таймер
      ///
        if (v:IsValid() and string.find( v:GetClass(), "prop_phys" )) then
              local dir = ( self:GetPos() - v:GetPos() + Vector(0, 0, 250) ):GetNormal()
              local force = 500
              local distance = dir:Length()
              ...
              local vForce = -1*dir * (force * ratio)
              local phys = v:GetPhysicsObject()
              phys:ApplyForceCenter( -dir * phys:GetMass() * 1000 )
        end
      ///

        if ( v:IsValid() and v:IsRagdoll() ) then
          if (!v:IsValid()) then  end
      //////////Кровь от тела//////
      ...
      util.Decal( "blood", v:GetPos() + Vector(math.random(25,35),math.random(25,35),-1), v:GetPos() - Vector(-math.random(25,35),-math.random(25,35),50))
              ...
              v:GetModel() == "models/spenser/ssk/new_sviter_spen.mdl" or
              v:GetModel() == "models/spenser/ssk/new_krovo_spen.mdl") then
          ...
          bone1:Spawn()
          bone1:Activate()
          util.Decal( "blood", bone1:GetPos() + Vector(-math.random(1,10),-math.random(1,10),-1), bone1:GetPos() - Vector(math.random(1,10),math.random(1,10),50))
        ...
        if (math.random(1, 10) < 9) then
          ...
          bone2:Spawn()
          bone2:Activate()
          util.Decal( "blood", bone2:GetPos() + Vector(-math.random(1,10),-math.random(1,10),-1), bone2:GetPos() - Vector(math.random(1,10),math.random(1,10),50))
          timer.Simple(10,function ()
          bone2:Remove()
          end)
        end
      ...
      local effectdata = EffectData()
            effectdata:SetStart( self:GetPos() + Vector( 0, 0, 200 )) 
            effectdata:SetOrigin( self:GetPos() + Vector( 0, 0, 200 ))
      effectdata:SetScale( 1550 )
    ...
    self.surge = CurTime() + 10
      ...
      if (v:IsPlayer()) then
        ///
        local client = v
        if (client:GetPos():Distance(self:GetPos()) <= 225 and client:GetPos():Distance(self:GetPos()) >= 151) then
          //
          ...
          //
        elseif (client:GetPos():Distance(self:GetPos()) <= 150 and client:GetPos():Distance(self:GetPos()) >= 81) then
          //
           if (client:Health() > 0) then
