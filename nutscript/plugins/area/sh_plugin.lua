...
if (SERVER) then
	...
	function playerMeta:isInAreaZone()
		netstream.Start(self, "hudsafezone", self.safezone)
	end
	...
	timer.Create("nutAreaController", 1, 0, function()
	...
	concommand.Add("tes", function(ply)
		--ply:ChatPrint(ply.curArea) -- при заходе в новую зону, новый id.
		ply:ChatPrint(tostring(ply.safezone))
		--netstream.Start(ply, "hudsafezone", ply.safezone)
	end)
	
	-- Если игрок Вне зоны
	function PLUGIN:OnPlayerAreaNoZone(client)
		--client:GodDisable()
		client.safezone = false
	end
	
	-- Если игрок находится в зоне
	function PLUGIN:OnPlayerAreaZone(client, areaID)
		--local areaData = nut.area.getArea(areaID)
		--client:GodEnable()
		client.safezone = true
	...
	netstream.Hook("areaAdd", function(client, name, vector1, vector2)
		-- Only Admin can edit the area.
		if (!client:IsSuperAdmin() or !vector1 or !vector2 or !name) then
			return false
		end
