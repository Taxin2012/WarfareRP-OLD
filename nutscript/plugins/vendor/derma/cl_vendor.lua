local PANEL = {}
	function PANEL:Init()
		...
		function self.vendorName:Paint( w, h )
			local material = Material("ui/money_ui.png") 
		...
		self.vendorName:SetFont("LFontForSafe")

		self.ourName = header:Add("DLabel")
		self.ourName:Dock(RIGHT)
		function self.ourName:Paint( w, h )
			local material = Material("ui/money_ui.png") 
			surface.SetDrawColor(color_white) 
			surface.SetMaterial(material) 
			surface.DrawTexturedRect(0, 0, w, h) 
		...
		self.ourName:SetFont("LFontForSafe")
		...
		self.vendorSell = footer:Add("DButton")  --кнопка покупка слева
		self.vendorSell:SetFont("LFontForSafe")
		self.vendorSell:SetWide(self.vendorName:GetWide())
		self.vendorSell:Dock(LEFT)
		--self.vendorSell:SetSize(135, 25)
		--self.vendorSell:SetPos(60, 0)
		function self.vendorSell:Paint( w, h )
		...
		self.vendorSell:SetTextColor(Color(210, 180, 140))
		...
		self.vendorBuy = footer:Add("DButton") --кнопка купить справа
		self.vendorBuy:SetFont("LFontForSafe")
		self.vendorBuy:SetWide(self.ourName:GetWide())
		--self.vendorBuy:SetSize(135, 25)
		--self.vendorBuy:SetPos(245, 0)
		self.vendorBuy:Dock(RIGHT)
		function self.vendorBuy:Paint( w, h )
			local material = Material("ui/trade_but.png") 
			surface.SetDrawColor(color_white) 
			surface.SetMaterial(material) 
			surface.DrawTexturedRect(0, 0, w, h) 
		...
		self.vendorBuy:SetTextColor(Color(210, 180, 140))
		...
		--[[self.vendorExit = footer:Add("DButton") --кнопка выйти
		self.vendorExit:SetFont("ChatFontMisery")
		self.vendorExit:SetWide(self.ourName:GetWide())
		self.vendorExit:SetSize(185, 25)
		self.vendorExit:SetPos(185, 20)
		function self.vendorExit:Paint( w, h )
			local material = Material("ui/trade_but.png") 
			surface.SetDrawColor(color_white) 
			surface.SetMaterial(material) 
			surface.DrawTexturedRect(0, 0, w, h) 
		end
		self.vendorExit:SetContentAlignment(5)
		self.vendorExit:SetText(L"Выход")
		self.vendorExit:SetTextColor(color_white)
		self.vendorExit.DoClick = function(this)
			self:Close()
		end]]
		...
		--[[function self.selling:Paint( w, h )
	...
	local material = Material("ui/universal.png")
...
PANEL = {}
	function PANEL:Init()
		...
		self.name:SetFont("ChatFontMisery")
