	...
	function PLUGIN:HUDPaint()
		...
		if (fraction > 0) then
			...
			for k, v in ipairs(weapons) do
				...
				--local color = Color(210, 180, 140, opacity)
				local color = ColorAlpha(k == self.index and Color(169,169,169,opacity) or Color(210, 180, 140, opacity), (255 - math.abs(theta * 3) * 255) * fraction)
				--local color = ColorAlpha(Color(169,169,169,opacity) or Color(210, 180, 140, opacity), (255 - math.abs(theta * 3) * 255) * fraction)
				local lastY = 0
				...
				surface.SetFont("BigFontForSafe")
				...
				cam.PushModelMatrix(matrix)
					nut.util.drawText(v:GetPrintName():upper(), 2, ty/2, color, 0, 1, "BigFontForSafe")
				cam.PopModelMatrix()
			end
